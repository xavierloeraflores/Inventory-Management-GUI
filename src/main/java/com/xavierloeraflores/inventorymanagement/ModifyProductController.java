...
public class ModifyProductController implements Initializable {
    ...
    public void mapTables(){
        ...
        tableAssociatedPart.setItems(selectedProduct.getAllAssociatedParts());
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        oldProduct = MainFormController.getSelectedProduct();

        productId = oldProduct.getId();
        String name = oldProduct.getName();
        Double  price = oldProduct.getPrice();
        int stock = oldProduct.getStock();
        int min = oldProduct.getMin();
        int max = oldProduct.getMax();
        selectedProduct = new Product(productId, name, price, stock,  min, max);
        for(int i =0; i <oldProduct.getAllAssociatedParts().size(); i++){
            selectedProduct.addAssociatedPart(oldProduct.getAllAssociatedParts().get(i));
        }
;

        fieldName.setText(selectedProduct.getName());
        fieldStock.setText(Integer.toString(selectedProduct.getStock()));
        fieldPrice.setText(Double.toString(selectedProduct.getPrice()));
        fieldMin.setText(Integer.toString(selectedProduct.getMin()));
        fieldMax.setText(Integer.toString(selectedProduct.getMax()));
        fieldId.setText(Integer.toString(productId));
        mapTables();
    }
}